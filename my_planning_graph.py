...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if all(precond in self.s_levels[level] for precond in a_node.prenodes):
                self.s_levels[level+1].update(a_node.effnodes)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_add in node_a1.action.effect_add:
            if node_add in node_a2.action.effect_rem:
                return True

        for node_rem in node_a1.action.effect_rem:
            if node_rem in node_a2.action.effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_add in node_a1.action.effect_add:
            if node_add in node_a2.action.precond_neg:
                return True

        for node_rem in node_a1.action.effect_rem:
            if node_rem in node_a2.action.precond_pos:
                return True

        for node_add in node_a2.action.effect_add:
            if node_add in node_a1.action.precond_neg:
                return True

        for node_rem in node_a2.action.effect_rem:
            if node_rem in node_a1.action.precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for node_pos in node_a1.action.precond_pos:
            if node_pos in node_a2.action.precond_neg:
                return True

        for node_neg in node_a1.action.precond_neg:
            if node_neg in node_a2.action.precond_pos:
                return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        a_s1 = list(node_s1.parents)
        a_s2 = list(node_s2.parents)
        if len(node_s1.parents) == 1 and len(node_s2.parents) == 1 and a_s1[0].is_mutex(a_s2[0]):
            return True
        return False
        ...
        for goal in self.problem.goal:
            ...
            for level, states in enumerate(self.s_levels):
                if is_goal:
                    ...
                    for state in states:
                        s_lit = expr(state.symbol)
                        if not state.is_pos:
                            s_lit = expr('~{}'.format(state.symbol))
                        if goal == s_lit:
                            level_sum += level
                            is_goal = True
                            break
